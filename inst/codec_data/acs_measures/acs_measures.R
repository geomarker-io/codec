...
out$n_persons_under_18 <-
  get_acs_5yr_data(c(paste0("B01001_00", 3:6, "E"), paste0("B01001_0", 27:30, "E"))) |>
  rowwise() |>
  mutate(
...
out$prop_health_insurance <-
  ...
  rowwise() |>
  mutate(
    prop_health_insurance = sum(c_across(-c(B27001_001E, census_tract_id_2020))) / B27001_001E,
    .keep = "unused"
  ) |>
  ungroup()
...
out$rent_burdened <-
  ...
  rowwise() |>
  mutate(
    prop_rent_burdened = sum(c_across(-c(B25070_001E, census_tract_id_2020))) / B25070_001E,
    .keep = "unused"
  ) |>
  ungroup()
...
out$housing_conditions <-
  ...
  rowwise() |>
  mutate(
    prop_housing_conditions = sum(c_across(-c(B25123_001E, census_tract_id_2020))) / B25123_001E,
    .keep = "unused"
  ) |>
  ungroup()
...
out$housing_age <-
  ...
  rowwise() |>
  mutate(
    prop_built_prior_1980 = sum(c_across(-c(B25034_001E, census_tract_id_2020))) / B25034_001E,
    .keep = "unused"
  ) |>
  ungroup()
...
out$language <-
  ...
  rowwise() |>
  mutate(
    prop_limited_english_speaking = sum(c_across(-c(C16002_001E, census_tract_id_2020))) / C16002_001E,
    .keep = "unused"
  ) |>
  ungroup()
...
out$edu <-
  ...
  rowwise() |>
  mutate(
    prop_adults_hs_edu = sum(c_across(-c(B15003_001E, census_tract_id_2020))) / B15003_001E,
    .keep = "unused"
  ) |>
  ungroup()
...
out_dpkg <-
  ...
  as_codec_dpkg(
    ...
    description = paste(readLines(fs::path_package("codec", "codec_data", "acs_measures", "README.md")), collapse = "\n")
  )
