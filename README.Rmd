...
Automagically add `name`, `type` and `enum` schema to each column in the data [based on their class](link to where this is documented -- vignette?)
...
Once our metadata is set correctly, we can save our tabular data resource as a YAML file:
...
We also need to save our CSV file using `readr::write_csv()` since it takes care of all of the CSV defaults for us?? (so we don't have to specify those in the schema -- just assume the defaults always)

This allows others to read in a CSV file in R and use the metadata file to properly define the classes (and levels) of the columns:
