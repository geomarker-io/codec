...
Automagically add `name`, `type` and `enum` schema to each column in the data:
...
This creates (1) creates a new folder based on the `name` attribute of our tibble, (2) writes the tabular data into a CSV file named based on the `name` attribute, and (3) extracts the metadata from the tibble's attributes and saves it in the frictionless tabular-data-resource specification as a yaml file:
