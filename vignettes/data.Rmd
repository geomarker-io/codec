...
make_core_md <- function(x = cd[[1]]) {
  md <- purrr::keep_at(as.list(x), c("name", "title", "homepage", "version", "description"))
  ...
  # TODO best way to directly show schema?
...
purrr::walk(cd_md,
            function(.) {
              glue::glue_data(
                ...
                ## "<hr>",
                "## {name_button}",
                ## "[**{title}**]({homepage}) {version} {dl_button}",
                "**{ifelse(length(title) == 0, name, title)}** {version} {dl_button}",
                "> {description}",
                ## "<hr>",
                ## "read into R with:",
                ## "```r",
