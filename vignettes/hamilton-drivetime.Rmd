---
title: "Hamilton County Tract-Level Drivetime to CCHMC"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Hamilton County Tract-Level Drivetime to CCHMC}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r setup, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)

library(CODECtools)
library(knitr)
options(knitr.kable.NA = '')
mapview::mapviewOptions(leafletWidth = 850)
```

```{r, include = FALSE}
d <- read_tdr_csv(glue::glue("https://github.com/geomarker-io/hamilton_drivetime/releases/download/v0.1.0")) |>
  dplyr::rename(census_tract_id_2010 = census_tract_id) |>
  dplyr::mutate(year = as.integer(2021)) |>
  add_col_attrs(year, name = "year", title = "Year", description = "data year") |>
  add_type_attrs()

write_tdr_csv(d, dir = "tmp")
check_codec_tdr_csv(fs::dir_ls("tmp"))
fs::dir_delete("tmp")
write_tdr_csv(d, "codec_data/")
```

**last updated:** `r gh::gh(glue::glue("/repos/geomarker-io/{glimpse_attr(d)$value[glimpse_attr(d)$name == 'name']}/releases"))[[1]]$created_at`

### Attributes

```{r, echo = FALSE}
glimpse_attr(d) |>
  knitr::kable()
```

### Schema

```{r, echo = FALSE}
glimpse_schema(d) |>
  knitr::kable()
```

### Data

```{r, echo = FALSE}
DT::datatable(d, 
              extensions = 'Buttons', 
              options = list(
                dom = 'Bfrtip',
                buttons = 'csv', 
                scrollX = TRUE)
)
```

### Map

```{r, echo = FALSE, message = FALSE}
library(sf)

d <- dplyr::left_join(d, 
                      cincy::tract_tigris_2010, 
                      by = "census_tract_id_2010") |>
  st_as_sf()

m <- mapview::mapview(d, zcol = colnames(d)[2:(ncol(d)-1)]) 

m@map |>
  leaflet::hideGroup(colnames(d)[3:(ncol(d)-1)])
```


