...
- the **schema** (or ["table schema"](https://specs.frictionlessdata.io/table-schema/)) contains information about each of the **fields** (or "columns") within a tabular-data-resource
  - **schema** may optionally contain the value used to denote missingness and which fields are foreign or primary keys

The names of any **descriptor** **schema** and **field** values MUST be one of:

```{r}
CODECtools::codec_names()
```

A CODEC `tdr` MUST contain `name` and `path` descriptors. All other descriptors, schema, and fields MAY be present.
